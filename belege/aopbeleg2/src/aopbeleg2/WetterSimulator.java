	...
	public void gewitternLassen() {
		...
		while (!getroffen) {
			for(int y = 1; y <= 5; y++) {
				for(char x = 'A'; x <= 'E'; x++) {
					Gegenstand gegenstand = raster.getGegenstand(x, y);
					if(gegenstand instanceof Gebaeude && !(gegenstand instanceof Wohngebaeude) && Math.random() < 0.05) {
						System.out.println(gegenstand.getTyp() + " an Position " + x + y + " wurde vom Blitz getroffen.");
						getroffen = true;
						break;
					}
				}
